...
namespace GUI.Forms
{
    public partial class FrmLogin : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            AccountDTO account = new AccountDTO(0, username, password);

            try
            {
                Account = _accountBUS.CheckLogin(username, password);

                MyMessageBox.ShowInformation($"Đăng nhập thành công\r\nXin chào {username}");

                Hide();

                using (GUI.Forms.FrmMain formMain = new GUI.Forms.FrmMain())
                {
                    formMain.ShowDialog();
                }
