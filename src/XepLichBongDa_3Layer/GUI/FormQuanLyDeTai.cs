...
namespace GUI
{
    public partial class FormQuanLyDeTai : Form
    {
        ...
        {
            if (ValidateDeTai(out string maDT, out string tenDT, out string chuNhiem, out long kinhPhi))
            {
        ...
        // Xử lý lưu sinh viên
        private void HandleLuuSinhVien(string maDT, string tenDT, string chuNhiem, long kinhPhi)
        {
            try
            {
                if (_luuThem)
        ...
        private bool ValidateDeTai(out string maDT, out string tenDT, out string chuNhiem, out long kinhPhi)
        {
            ...
            // Kiểm tra Kinh phí
            if (!long.TryParse(txtKinhPhi.Text.Trim(), out kinhPhi) || kinhPhi < 0)
            {
